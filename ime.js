...
function AjaxIME(doc) {
  ...
  function ImeSetCaretPos(pos) {
    if (ImeTextArea_.setSelectionRange) {
      // Mozilla
      ImeTextArea_.setSelectionRange(pos, pos);
    } else if (ImeDocument_.selection.createRange) {
      // IE
  ...
  function ImeStoreWindowPos() {
    ...
    if (ImeTextArea_.selectionStart || ImeTextArea_.selectionStart == "0") {
      // Mozilla
  ...
  function ImeInsertText(text, delay) {
    ...
    if (ImeTextArea_.createTextRange && ImeTextArea_.caretPos) {
      ...
      ImeTextArea_.value =
        ImeTextArea_.value.substring(0, ImeStartPos_) +
        text +
        ImeTextArea_.value.substring(ImeEndPos_, ImeTextArea_.value.length);
      callWithDelay(func, delay);
  ...
  function ImeAdd(text, selected) {
    ...
    if (text.length > ImePreEditWidth_) ImePreEditWidth_ = text.length;
  ...
  function ImePreEditKeyDown(event) {
    ...
    if (key == 0x20) {
      // space
      ImeTo_ = "ime";
      if (ImeIsHidden()) ImeShowCandidates();
      else ImeHandleDown();
    } else if (key == 120) {
      // F9 katakana
      ImeTo_ = "katakana";
  ...
  function ImePreEditKeyUp(event) {
    ...
    if (ImePreEdit_.value.length > ImePreEditWidth_) ImePreEditWidth_ = ImePreEdit_.value.length;
  ...
  function ImeChangeMode() {
    ...
    var textareas = ImeDocument_.getElementsByTagName("textarea");
    for (var i = 0; i < textareas.length; ++i) {
      if (!textareas[i].__ImeUsed) textareas[i].style.backgroundColor = ImeMode_ ? ImeBackGroundColor_ : "white";
    }
  }
  ...
  function ImeTextAreaKeyDown(event) {
    ...
    if (
      ImeMode_ &&
      ImeIsHidden() &&
      ((key >= 65 && key <= 90) || (key >= 48 && key <= 57) || (key >= 187 && key <= 222))
    ) {
      ImeShowPreEdit();
      return false;
    }
...
function roma2hiragana(str, delay) {
  ...
  for (var i = 0; i < text.length; ) {
    ...
    var len = 0;
    if ((c >= 97 && c <= 122) || (c >= 65 && c <= 90) || (c >= 44 && c <= 46)) len = 4;
    while (len) {
